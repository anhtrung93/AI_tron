...
// ===========================================================
var fs = require('fs');
var testNumber = +((fs.readFileSync('testNumber.txt')).toString());
++ testNumber;
fs.writeFileSync('testNumber.txt', "" + testNumber);
...
function findMaxAfterSplit(board, pos){ 
  ...
  //console.error("estimate longest = " + longestEst[0]);
  ////console.error("move = " + longestEst[1]);
  return longestEst[1];
}
...
function minimax(board, myPos, enemyPos, depthLvl, depthLimitLvl, alpha, beta){
  ...
  if (Math.abs(value) >= 200){ 
    string += (value % 100) + "\n";
    fs.appendFile('input/input' + ((testNumber < 10)? "0" : "") + testNumber + '.txt', string, function(err){
      if (err){
        throw err;
      }
    });
...
exports.findMaxAfterSplit = findMaxAfterSplit;
